...
public class Main extends JFrame implements MouseListener
{
	...
	private static final int Height=700; //screen height
	private static final int Width=1200; //screen width
	private static Rook wr01,wr02,br01,br02; //two black two white rooks
	private static Knight wk01,wk02,bk01,bk02; //two white two black knights
	private static Bishop wb01,wb02,bb01,bb02;
	private static Pawn wp[],bp[];
	private static Queen wq,bq;
	private static King wk,bk;
	private Tile c,previous; //current and previous tiles
	private static final long serialVersionUID = 1L;
	private int chance=0;
	private Tile boardState[][];
	private ArrayList<Tile> destinationlist = new ArrayList<Tile>();
	...
	private ArrayList<String> Wnames=new ArrayList<String>(); //name list
	...
	private Button start,wselect,bselect,WNewPlayer,BNewPlayer;    //button details
	public static int timeRemaining=60;
	public static void main(String[] args){
	
	//variable initialization
	wr01=new Rook("WR01","White_Rook.png",0); //id name,path for img,color
	...
	wk=new King("WK","White_King.png",0,7,3); //extra variables for king's position
	...
	private Main()
    {
		...
		move="White"; //starting move for white
		wname=null;
		bname=null;
		winner=null;
		...
		timeSlider.setMaximum(10); //maximum 10 minutes for each move
		...
		while(witr.hasNext())
			Wnames.add(witr.next().name()); //list of players
				
		bplayer= Player.fetch_players();
		Iterator<Player> bitr=bplayer.iterator();
		while(bitr.hasNext())
			Bnames.add(bitr.next().name()); //list of players
	...
	// public so that can be accessed from Time Class
	...
	//A function to clean the highlights of possible destination tiles
    private void cleandestinations(ArrayList<Tile> destlist)      //Function to clear the last move's destinations
    {
    	ListIterator<Tile> it = destlist.listIterator();
    	while(it.hasNext())
    		it.next().removepossibledestination();
    }
    
    //A function that indicates the possible moves by highlighting the tiles
    private void highlightdestinations(ArrayList<Tile> destlist)
    {
    	ListIterator<Tile> it = destlist.listIterator();
    	while(it.hasNext())
  ...
  //Function to check if the king will be in danger if the given move is made
    private boolean willkingbeindanger(Tile fromcell, Tile tocell)
    {
    	Tile newboardstate[][] = new Tile[8][8];
    	for(int i=0;i<8;i++)
    		for(int j=0;j<8;j++)
    		{	try { newboardstate[i][j] = new Tile(boardState[i][j]);} catch (CloneNotSupportedException e){e.printStackTrace(); System.out.println("There is a problem with cloning !!"); }}
    	
    	if(newboardstate[tocell.row][tocell.col].getpiece()!=null)
			newboardstate[tocell.row][tocell.col].removePiece();
    	
		newboardstate[tocell.row][tocell.col].setPiece(newboardstate[fromcell.row][fromcell.col].getpiece());
		if(newboardstate[tocell.row][tocell.col].getpiece() instanceof King)
		{
			((King)(newboardstate[tocell.row][tocell.col].getpiece())).setx(tocell.row);
			((King)(newboardstate[tocell.row][tocell.col].getpiece())).sety(tocell.col);
		}
		newboardstate[fromcell.row][fromcell.col].removePiece();
    ...
    private ArrayList<Tile> filterdestination (ArrayList<Tile> destlist, Tile fromcell)
    {
    	...
    	while (it.hasNext())
    	{
    		for(int i=0;i<8;i++)
        		for(int j=0;j<8;j++)
        		{	try { newboardstate[i][j] = new Tile(boardState[i][j]);} catch (CloneNotSupportedException e){e.printStackTrace();}}
    		
    		Tile tempc = it.next();
    		if(newboardstate[tempc.row][tempc.col].getpiece()!=null)
    			newboardstate[tempc.row][tempc.col].removePiece();
    		newboardstate[tempc.row][tempc.col].setPiece(newboardstate[fromcell.row][fromcell.col].getpiece());
    		x=getKing(chance).getx();
    		y=getKing(chance).gety();
    		if(newboardstate[fromcell.row][fromcell.col].getpiece() instanceof King)
    		{
    			((King)(newboardstate[tempc.row][tempc.col].getpiece())).setx(tempc.row);
    			((King)(newboardstate[tempc.row][tempc.col].getpiece())).sety(tempc.col);
    			x=tempc.row;
    			y=tempc.col;
    		}
    		newboardstate[fromcell.row][fromcell.col].removePiece();
    ...
    private ArrayList<Tile> incheckfilter (ArrayList<Tile> destlist, Tile fromcell, int color)
    {
    	ArrayList<Tile> newlist = new ArrayList<Tile>();
    	Tile newboardstate[][] = new Tile[8][8];
    	ListIterator<Tile> it = destlist.listIterator();
    	...
    	while (it.hasNext())
    	{
    		...
    		if(newboardstate[tempc.row][tempc.col].getpiece() instanceof King)
    		{
    			((King)(newboardstate[tempc.row][tempc.col].getpiece())).setx(tempc.row);
    			((King)(newboardstate[tempc.row][tempc.col].getpiece())).sety(tempc.col);
    			x=tempc.row;
    ...
    {
    	ArrayList<Tile> dlist = new ArrayList<Tile>();
    	for(int i=0;i<8;i++)
    	{
    		for(int j=0;j<8;j++)
    		{
    ...
    //which is called when the user clicks on a particular tile
	@Override
	public void mouseClicked(MouseEvent arg0)
	{
		c=(Tile)arg0.getSource();
		if (previous==null)
		{
			if(c.getpiece()!=null)
			{
				...
				destinationlist=c.getpiece().move(boardState, c.row, c.col);
				if(c.getpiece() instanceof King)
					destinationlist=filterdestination(destinationlist,c);
				else
				{
					if(boardState[getKing(chance).getx()][getKing(chance).gety()].ischeck())
						destinationlist = new ArrayList<Tile>(filterdestination(destinationlist,c));
		...
		{
			if(c.row ==previous.row && c.col ==previous.col)
			{
				c.deselect();
				cleandestinations(destinationlist);
				destinationlist.clear();
			...
			else if(c.getpiece()==null||previous.getpiece().getcolor()!=c.getpiece().getcolor())
			{
				if(c.ispossibledestination())
				{
					...
					if(c.getpiece() instanceof King)
					{
						((King)c.getpiece()).setx(c.row);
						((King)c.getpiece()).sety(c.col);
					}
					changechance();
					if(!end)
					{
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if(White==null||Black==null) //if player is not selected
